...
class Cross_attLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        # The cross-attention Layer
        self.visual_attention = BertCrossattLayer(config)
...
class CrossEncoder(nn.Module):
    def __init__(self,config):
        super().__init__()
        self.num_cross_layers = config.cross_att_layer_num
        self.cross_layers = nn.ModuleList(
            [Cross_attLayer(config) for _ in range(self.num_cross_layers)]  )

    def forward(self, lang_feats, visn_feats,lang_attention_mask=None,
